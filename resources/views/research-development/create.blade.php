<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Create Research & Development Project') }}
        </h2>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    <form action="{{ route('research-development.store') }}" method="POST">
                        @csrf
                            ...
                            <div>
                                ...
                                <select name="lead_researcher_id" id="lead_researcher_id"
                                    ...
                                    @foreach($researchers as $researcher)
                                        <option value="{{ $researcher->id }}" {{ old('lead_researcher_id') == $researcher->id ? 'selected' : '' }}>
                                            {{ $researcher->name }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="mt-6">
                            ...
                            <textarea name="description" id="description" rows="4"
                                      ...
                                      placeholder="Describe the research project, objectives, methodology, and expected outcomes..." required>{{ old('description') }}</textarea>
                            @error('description')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
